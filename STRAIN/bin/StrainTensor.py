#! /usr/bin/python
#-*- coding: utf-8 -*-
#python ./StrainTensor.py -i ../data/CNRS_midas.vel -r 18.75/30.25/32.75/42.25 --x-grid-step=0.5 --y-grid-step=0.5 --dmin=1 --dmax=500 --dstep=1 --Wt=24 -c -g
...
from math import degrees, radians, floor, ceil# из радиан в град биз гр в радианныб наибольшее целое число, меньшее или равное x ,наименьшее целое число, большее или равное x 
import numpy
...
# падает на вход ограничение окна в координатах
def cut_rectangle(xmin, xmax, ymin, ymax, sta_lst, sta_list_to_degrees=False):
   # print('oleg ', sta_lst, sta_list_to_degrees)
...
def compute__(igrd, sta_list_utm, utm_lcm, fout, fstats, vprint_fun, **dargs):
    ...
    for x, y in igrd:# Gradus
        ...
        sstr = ShenStrain(E, N, clat<0e0, sta_list_utm, **dargs)# экземпляр класса
        ## check azimouth coverage (aka max β angle)
        if degrees(max(sstr.beta_angles())) <= dargs['max_beta_angle']:
            try:
                sstr.estimate()# метод расчета до которого еще куча методов
...
parser.add_argument('-m', '--method',
    ...
    choices=['shen', 'veis'],
    required=False,
    help='Choose a method for strain estimation. If \'shen\' is passed in, the estimation will follow the algorithm described in Shen et al, 2015, using a weighted least squares approach. If \'veis\' is passed in, then the region is going to be split into delaneuy triangles and a strain estimated in each barycenter.Выберите метод оценки деформации. Если передать \'shen\', оценка будет следовать алгоритму, описанному в Shen et al, 2015, с использованием метода взвешенных наименьших квадратов. Если передать \'veis\', то область будет разделена на треугольники Делане и деформация оценивается в каждом барицентре.')
...
parser.add_argument('-t', '--weighting-function',
    ...
    required=False,
    help='Only relevant for \'--mehod=shen\'. Choose between a \'gaussian\' or a \'quadratic\' spatial weighting function.Актуально только для \'--mehod=shen\'. Выберите между гауссовой или квадратичной функцией пространственного взвешивания.')
...
parser.add_argument('--Wt',
    ...
    type=int,
    required=False,
    help='Only relevant for \'--mehod=shen\' and if \'d-param\' is not passed in. Let W=Σ_i*G_i, the total reweighting coefficients of the data, and let Wt be the threshold of W. For a given Wt, the smoothing constant D is determined by Wd=Wt . It should be noted that W is a function of the interpolation coordinate, therefore for the same Wt assigned, D varies spatially based on the in situ data strength; that is, the denser the local data array is, the smaller is D, and vice versa.Только для shen\ и если \d-param\' не передается. Пусть W=Σ_i*G_i, общие коэффициенты повторного взвешивания данных, и пусть Wt будет порогом W Для данного Wt постоянная сглаживания D определяется соотношением Wd=Wt. Следует отметить, что W является функцией интерполяционной координаты, поэтому для одного и того же назначенного Wt D варьируется в пространстве в зависимости от мощности данных in situ; то есть чем плотнее локальный массив данных, тем меньше D, и наоборот')
...
parser.add_argument('--dmin',
    ...
    type=int,
    required=False,
    help='Only relevant for \'--mehod=shen\' and if \'d-param\' is not passed in. This is the lower limit for searching for an optimal D-parameter value. Unit is km.. Это нижний предел поиска оптимального значения D-параметра. Единица км')
...
parser.add_argument('--dmax',
    ...
    type=int,
    required=False,
    help='Only relevant for \'--mehod=shen\' and if \'d-param\' is not passed in. This is the upper limit for searching for an optimal d-param value. Unit is km.. Это верхний предел поиска оптимального значения D-параметра. Единица км')
...
parser.add_argument('--dstep',
    ...
    type=int,
    required=False,
    help='Only relevant for \'--mehod=shen\' and if \'d-param\' is not passed in. This is the step size for searching for an optimal d-param value. Unit is km.Это размер шага для поиска оптимального значения d-параметра. Единица км.')
...
parser.add_argument('--d-param',
    ...
    type=float,
    required=False,
    help='Only relevant for \'--mehod=shen\'. This is the \'D\' parameter for computing the spatial weights. If this option is used, then the parameters: dmin, dmax, dstep and Wt are not used.Параметр \'D\' для вычисления пространственных весов. Если используется эта опция, то параметры: dmin, dmax, dstep и Wt не используются.')

parser.add_argument('-g', '--generate-statistics',
    dest='generate_stats',
    help='Only relevant when \'--mehod=shen\' and \'--barycenter\' is not set. This option will create an output file, named \'strain_stats.dat\', where estimation info and statistics will be written.не установлен. Эта опция создаст выходной файл с именем \'strain_stats.dat\', в который будет записана информация об оценке и статистика',
    action='store_true')

parser.add_argument('--verbose',
    dest='verbose_mode',
    help='Run in verbose mode (show debugging messages)Запускать в подробном режиме (показывать отладочные сообщения',
    action='store_true')

parser.add_argument('--multicore',
    dest='multiproc_mode',
    help='Run in multithreading mode Запуск в многопоточном режиме',
    action='store_true')
...
if __name__ == '__main__':
    ...
    dargs = vars(args) # Получаем аргументы
    ...
    ## Verbose print (function only exists in verbose mode) Подробная печать (функция существует только в подробном режиме
    vprint = print if args.verbose_mode else lambda *a, **k: None

    ## if in mutlithreading mode, load the module если в режиме многопоточности, загрузить модуль
...
##########################################################################################################
    ## If needed, open a file to write model info and statistics При необходимости откройте файл для записи информации о модели и статистики.
    ...
    ##+ m/yr.
    if not os.path.isfile(args.gps_file):# название входного файла присвоенного классом парсерром
        print('[ERROR] Cannot find input file \'{}\'.'.format( # если не найден фаил
            args.gps_file), file=sys.stderr)
        sys.exit(1)
    try:# вызывает функцию parse_ascii_input() в ней вызов класса Station.py и тупо проверка имя==имя lon==lon ругань
        sta_list_ell = parse_ascii_input(args.gps_file, args.method=='shen')# args.gps_file объект парсера с данными входного файла
        #for i in sta_list_ell:
            #в классе Station.py 'name', 'lat', 'lon', 've', 'vn', 'se', 'sn', 'rho', 't'Входныке данные переводит градусы в радианы скорости и ошибки делит на 1e3 т.е. +0.0 
            #print('input file  ',i.name)
            #print('input file  ',i.ve,' ',i.vn)
            #print('input file  ',i.se,' ',i.sn)
            #print('input file  ',i.rho,' ',i.t)
    ...
    if 'region' in args:
        try:
            ...
            # -r 18/30/32/42 
            if args.cut_outoflim_sta or args.method == 'veis': #из парсера класа берет значение или veis
                Napr = len(sta_list_ell)# количество строк с данными во входном файле
                #print(Napr)
    ...
    if 'region' in args and not args.method == 'veis' and not args.cut_outoflim_sta:
        #print('net opci -c i method ne veis')
        vprint('[DEBUG] Filtering stations based on their distance from region barycentre.')
        Napr = len(sta_list_ell)
        mean_lon, mean_lat = radians(lonmin+(lonmax-lonmin)/2e0), radians(latmin+(latmax-latmin)/2e0)#делает среднее из огранич квадрата
        bc =  Station(lon=mean_lon, lat=mean_lat)
        #print('mean lon lat ',mean_lon, mean_lat)
        #print(bc.__dict__)
        #print('bc obj ',bc.lon)
        endpt = Station(lon=radians(lonmax), lat=radians(latmax))
        #print(endpt.__dict__)
        #cutoffdis = endpt.h_d(bc)
        cutoffdis = abs(endpt.haversine_distance(bc)/1e3) # barycentre to endpoint (km)Вычисляет дугу в радианах между двумя точками на сфере
        #cutoffdis = endpt.haversine_distance(bc) # barycentre to endpoint (km)Вычисляет дугу в радианах между двумя точками на 
        print('cat of diss ',cutoffdis)
